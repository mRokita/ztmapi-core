...
{
	ScheduleManager* schedManager = new ScheduleManager();
	...
	TYsection->processLine("   WW   WSZYSTKICH ŒWIÊTYCH\r");
	REQUIRE(schedManager->dayTypes[2].id == "WW");
	REQUIRE(schedManager->dayTypes[2].name == "WSZYSTKICH ŒWIÊTYCH");
...
TEST_CASE("Sekcja KA", "[KA]")
{
	...
	REQUIRE(schedManager->getDayType("N44") == "NS");
...
TEST_CASE("Sekcja ZA", "[ZA]")
{
	...
	line = "   3344   Agatowa,                            JS  JÓZEFOS£AW";
	ZAsection->processLine(boostEncode(line));
	...
	REQUIRE(schedManager->stopGroups[4].regionName == boostEncode("JÓZEFOS£AW"));

	line = "   1951   Akacjowa,                           NM  NOWY DWÓR MAZ.";
	ZAsection->processLine(boostEncode(line));
	...
	REQUIRE(schedManager->stopGroups[5].regionName == boostEncode("NOWY DWÓR MAZ."));
	...
	line = "   1301   al.\"Solidarnoœci\",                  --  WARSZAWA";
	ZAsection->processLine(boostEncode(line));
	REQUIRE(schedManager->stopGroups[7].id == 1301);
	REQUIRE(schedManager->stopGroups[7].name == boostEncode("al.\"Solidarnoœci\""));
	...
	REQUIRE(schedManager->stopGroups[8].name == boostEncode("al.\"Solidarnoœci\""));
	...
	line = "   1888   Beniaminówka,                       W4  WÓLKA RADZYMIÑSKA";
	ZAsection->processLine(boostEncode(line));
	REQUIRE(schedManager->stopGroups[11].id == 1888);
	REQUIRE(schedManager->stopGroups[11].name == boostEncode("Beniaminówka"));
	REQUIRE(schedManager->stopGroups[11].regionId == "W4");
	REQUIRE(schedManager->stopGroups[11].regionName == boostEncode("WÓLKA RADZYMIÑSKA"));
...
TEST_CASE("Sekcja PR", "[PR]")
{
	...
	line = "         100304   4      Ul./Pl.: al.\"Solidarnoœci\",                Kier.: Szwedzka,                         Y= 52.255099     X= 21.036174     Pu=0";
	PRsection->processLine(boostEncode(line));
	REQUIRE(schedManager->stops[3].id == 100304);
	REQUIRE(schedManager->stops[3].groupId == 1003);
	REQUIRE(schedManager->stops[3].idInGroup == "04");
	REQUIRE(schedManager->stops[3].street == boostEncode("al.\"Solidarnoœci\""));
	...
	line = "         100402   1      Ul./Pl.: Jagielloñska,                     Kier.: pl.Hallera,                       Y= 52.257011     X= 21.028103     Pu=0";
	PRsection->processLine(boostEncode(line));
	REQUIRE(schedManager->stops[4].id == 100402);
	REQUIRE(schedManager->stops[4].groupId == 1004);
	REQUIRE(schedManager->stops[4].idInGroup == "02");
	REQUIRE(schedManager->stops[4].street == boostEncode("Jagielloñska"));
